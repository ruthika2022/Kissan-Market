  ...
  tickets: [
    {
      ...
      user: "Ravi Kumar",
      role: "Farmer",
      issue: "Payment delayed for PRD301",
      priority: "High",
      status: "Pending",
    },
    {
  ...
  complaints: [
    {
      ...
      user: "AgriCorp Ltd.",
      type: "Quality Issue",
      desc: "Received inferior quality wheat compared to sample.",
      status: "Under Review",
    },
    {
      id: "CMP502",
      user: "Gurpreet Singh",
      type: "Platform Bug",
      desc: "App crashes when uploading crop photos.",
      status: "Pending",
    },
  ],
  notifications: [
    {
      id: "N1",
      text: "New support ticket from Ravi Kumar",
      time: "10 mins ago",
      type: "ticket",
    },
    {
      id: "N2",
      text: "Low inventory alert: Organic Tomatoes",
      time: "1 hour ago",
      type: "alert",
    },
    {
      id: "N3",
      text: "Product PRD302 has been flagged",
      time: "2 hours ago",
      type: "warning",
    },
  ],
...
function loadCombinedFarmers() {
  const storedFarmers =
    JSON.parse(localStorage.getItem("km_farmers")) || initData.farmers;
  const profiles = JSON.parse(localStorage.getItem("km_profiles")) || {};
  const profileFarmers = Object.values(profiles).filter(
    (p) => p.role === "farmer",
  );
...
function loadCombinedVendors() {
  const storedVendors =
    JSON.parse(localStorage.getItem("km_vendors")) || initData.vendors;
  const profiles = JSON.parse(localStorage.getItem("km_profiles")) || {};
  const profileVendors = Object.values(profiles).filter(
    (p) => p.role === "vendor",
  );
...
let db = {
  ...
  inventory:
    JSON.parse(localStorage.getItem("km_inventory")) || initData.inventory,
  tickets: JSON.parse(localStorage.getItem("km_tickets")) || initData.tickets,
  complaints:
    JSON.parse(localStorage.getItem("km_complaints")) || initData.complaints,
  notifications:
    JSON.parse(localStorage.getItem("km_notifications")) ||
...
function adminSelectBid(bidId) {
  if (
    confirm(
      "Are you sure you want to select this bid? This will approve it, notify the farmer, and reject all other bids.",
    )
  ) {
...
function removeAdminProduct(productId) {
  if (
    confirm(
      "Are you sure you want to remove this product? This will permanently delete it.",
    )
  ) {
...
function renderProducts() {
  ...
  tbody.innerHTML = realProducts
    .map((p, i) => {
      const productBids = allBids.filter((b) => b.productId === p.id);
      const farmerProfile = (JSON.parse(localStorage.getItem("km_profiles")) ||
        {})[p.farmerId] || {
...
function renderTickets() {
  ...
  container.innerHTML = db.tickets
    .map(
      (t, i) => `
        <div class="info-card">
...
function renderComplaints() {
  ...
  container.innerHTML = db.complaints
    .map(
      (c, i) => `
        <div class="info-card">
            <div class="info-card-header">
...
function renderNotifications() {
  ...
  list.innerHTML = adminNotifs
    .map((n) => {
      let icon = "fa-bell";
      if (n.title.includes("Rejected") || n.title.includes("Declined"))
        icon = "fa-triangle-exclamation";
      else if (n.title.includes("Accepted") || n.title.includes("Approved"))
...
function suspendFarmer(idx) {
  if (
    confirm(`Are you sure you want to suspend farmer ${db.farmers[idx].name}?`)
  ) {
...
function removeProduct(idx) {
  if (
    confirm(
      "Warning: This will permanently remove the product from the marketplace. Proceed?",
    )
  ) {
...
function alertInventory(idx) {
  showToast(
    `Alert sent to ${db.inventory[idx].warehouse} for ${db.inventory[idx].name}`,
  );
}
...
window.addEventListener("click", function (e) {
  if (
    !document.getElementById("notifIcon").contains(e.target) &&
...
function updateCounters() {
  document
    .getElementById("countFarmers")
    .setAttribute("data-target", db.farmers.length);
  document
    .getElementById("countVendors")
    .setAttribute("data-target", db.vendors.length);
  document
    .getElementById("countProducts")
    .setAttribute(
      "data-target",
      db.products.filter((p) => p.pStatus === "Approved").length,
    );
//   document
//     .getElementById("countComplaints")
//     .setAttribute(
//       "data-target",
...
function resetData() {
  if (
    confirm(
      "This will restore all data to default and reload the page. Proceed?",
    )
  ) {
    localStorage.clear();
    location.reload();
